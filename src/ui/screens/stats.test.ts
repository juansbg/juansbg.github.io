...
describe('the ledger', () => {
  it.each(LOCALES)('adds the table up and lists every name once, most games first (%s)', (locale) => {
    ...
    expect(flat).toContain(`1 ${t.table.town(1)} 50%`)
    expect(flat).toContain(`1 ${t.table.family(1)} 50%`)
    ...
    expect(beto).toContain(`2 ${t.columns.games(2)}`)
    expect(beto).toContain(`2 ${t.columns.wins(2)}`)
    expect(beto).toContain(`1 ${t.columns.family(1)}`)
    expect(beto).toContain(`2 ${t.columns.survived(2)}`)
    // Ana was hanged once and killed once.
    const ana = text(html.slice(html.indexOf('ledger__row'), html.indexOf('Beto')))
    expect(ana).toContain(`1 ${t.columns.hanged(1)}`)
    expect(ana).toContain(`1 ${t.columns.killed(1)}`)
    // One of everything reads in the singular, in both languages.
    expect(t.columns.games(1)).not.toBe(t.columns.games(2))
    expect(t.columns.wins(1)).not.toBe(t.columns.wins(2))
    expect(t.columns.family(1)).not.toBe(t.columns.family(2))
